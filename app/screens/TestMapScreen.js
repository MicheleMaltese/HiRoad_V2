...
function TestMapScreen(props) {
    ...
    const [markers, setMarkers] = useState(
        currUser.maps[props.route.params.id].pins == undefined
            ? []
            : currUser.maps[props.route.params.id].pins
    );
    let [routes, setRoutes] = useState(
        currUser.maps[props.route.params.id].routes == undefined
            ? []
            : currUser.maps[props.route.params.id].routes
    );
    let [coordsArr, setCoordsArr] = useState([]);
    //Use as initial region, and also to reset the camera
    const [region, setRegion] = useState({
        latitude: currUser.maps[props.route.params.id].startlat,
        longitude: currUser.maps[props.route.params.id].startlng,
    ...
    const handleAddPin = async (coordinate, props) => {
        ...
        if (pinState && editMode) {
            ...
            props.navigation.navigate("Create Pin", {
                ...
                id: currUser.maps[props.route.params.id].pins.length - 1,
                mode: "add",
            });
        }
    };
    ...
    let handleDeleteRoute = (index) => {
        deleteRoute(props.route.params.id, index);
        let tempRoutes = routes;
    ...
    panResponder = PanResponder.create({
        ...
        onPanResponderRelease: async (e) => {
            if (routeState) {
                addRoute(props.route.params.id, {
                    color: "#000000",
                    label: "",
                    date: "",
                    coordsArr: coordsArr,
                ...
                props.navigation.navigate("Create Route", {
                    mapId: props.route.params.id,
                    id: currUser.maps[props.route.params.id].routes.length - 1,
                    mode: "add",
                });
            }
        },
