...
function CreatePinScreen(props) {
  ...
  const goBackUndoPin = () => {
    if (mode == "add") {
      deletePin(
        ...
        currUser.maps[props.route.params.mapId].pins.length - 1
      );
    }
  ...
  const goBackDeletePin = (index) => {
    if (mode == "edit") {
      deletePin(props.route.params.mapId, index);
      updateUserInfo();
    }
    props.navigation.goBack();
  };
  ...
  const [dateText, setDateText] = useState(
    currUser.maps[props.route.params.mapId].pins[props.route.params.id].date
  );
  ...
  const onConfirm = ({ date, dateString }) => {
    setShowDatePicker(false);
    setDate(date); // Update the Date object
    setDateText(dateString); // Update the displayed date string
  };
  ...
  const updatePinData = (props) => {
    if (dateFormatError.length > 0) {
      ...
      changeTitle(title, props.route.params.mapId, props.route.params.id);
      changeDesc(notes, props.route.params.mapId, props.route.params.id);
      changeW3wAddress(w3wAddress, props.route.params.mapId, props.route.params.id);
      changeArFlag(arEnabled, props.route.params.mapId, props.route.params.id);
      changeDate(
        dateText,
        props.route.params.mapId,
  ...
  useEffect(() => {
    ...
    /*} else if (userDateTypedInput.length === 0) {
            ...
            setDateFormatError(
                "Make sure the date is valid & in MM-DD-YYYY format."
            );
        }*/
    return () => {
  ...
  }, [userDateTypedInput, userDateValid, setUserDateValid, setDateFormatError]);
